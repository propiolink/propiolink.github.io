...
if(tq==0){tq=1; setTimeout("tq=0",500)
  ...
  salid=`<div onclick="if(window.confirm('Seguro que desea borrar este elemento no se puede desacer') && obj.id!='blok'){obj.remove();opt.style.display='none';grd()}" class="btns bgr">Delete</div>   <div onclick="opt.style.display='none'" class="btns bgv">Done</div>`
...
function adds(nu){
//PÁRRAFO 
  if(nu==0){text=`\n<p class="elem">tu párrafo</p>\n`} 
//POST
  if(nu==1){text=`\n<div class="elem" style="width:96%;padding:10px 5px;margin:2%;border-radius:5px;box-shadow:0px 0px 10px #000;"><p style="font-weight:bolder;font-size:1.6em" class="elem">tu título</p><p style="" class="elem">tu párrafo</p></div>\n`} 
...
ctl()}
  ...
  function usar(){
  if(localStorage.us!=""){copyTo();openPage(`https://quintadb.com/apps/ddPsyqytvcROk-fKOxW5vX/dtypes/${localStorage.id}/edit?entity_id=cmWPT4mSjaxik9amkih8oz?&overal_dtypes_size=1&widget=true`)}
  }
function copyTo(){ 
